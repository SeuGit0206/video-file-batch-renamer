...
- **自動品番抽出 & 柔軟なファイル追加**: 多様な命名パターンから正規表現アルゴリズムを用いて作品ID（例: `ABC-123`, `FC2-PPV-102934`）を自動抽出。ドラッグ＆ドロップに加え、OS標準のファイル選択ダイアログおよび手動ファイル名入力に対応。
- **実ファイルリネームスクリプト出力**: Windows PowerShell (`.ps1`) および Windows バッチ (`.bat`) スクリプトを自動生成。ワンクリックでクリップボードへコピーまたはファイル保存し、ローカル環境で安全に一括リネームを実行可能。
- **安全保護 & 品番フォールバック**: 特殊記号（`[1080p]`等の角括弧やシングルクォート）のエスケープ、日本語文字化け防止（UTF-8 BOM）、リネーム先重複時の上書き防止、メタデータ取得失敗時の品番ベース安全リネームを完備。
...
- **リアルタイムシミュレーション & プレビュー**: リネーム実行前に変更前後の比較プレビュー、リネームルール設定、衝突チェック、トランザクション Undo/Redo が可能。
